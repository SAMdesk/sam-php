...
class SAM_Object implements ArrayAccess
{
  ...
  public static function init() {
    self::$_permanentAttributes = new SAM_Util_Set( array( "_auth" ) );
  ...
  public function refreshFrom( $values, $auth, $partial=false ) {
    ...
    foreach ( $values as $k => $v ) {
      ...
      if ( self::$_nestedUpdatableAttributes->includes( $k ) && is_array( $v ) )
        $this->_values[$k] = SAM_Object::scopedConstructFrom( "SAM_AttachedObject", $v, $auth );
      else
        $this->_values[$k] = SAM_Util::convertToSAMObject( $v, $auth );
